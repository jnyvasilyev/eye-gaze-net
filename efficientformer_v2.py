...
class Embedding(nn.Module):
    def __init__(self, patch_size=3, stride=2, padding=1,
                 in_chans=5, embed_dim=768, norm_layer=nn.BatchNorm2d,
...
class EfficientFormerV2(nn.Module):
    def __init__(self, layers, embed_dims=None,
                 ...
                 pretrained=None,
                 vit_num=0,
                 resolution=64,  # Adjusted resolution
        ...
        self.patch_embed = stem(5, embed_dims[0], act_layer=act_layer)
        ...
        self.maps = nn.Conv2d(embed_dims[-1], 3, kernel_size = 1)
        ...
        if self.fork_feat:
            ...
            self.flow_brightness_head = nn.Conv2d(embed_dims[-1], 3, kernel_size=1)
            trunc_normal_(self.flow_brightness_head.weight, std=.02)
            if self.flow_brightness_head.bias is not None:
                nn.init.constant_(self.flow_brightness_head.bias, 0)
    ...
    def forward_tokens(self, x):
        ...
        for idx, block in enumerate(self.network):
            ...
            if self.fork_feat and idx in self.out_indices:
                ...
                outs.append(x_out)
        if self.fork_feat:
            flow_brightness = self.flow_brightness_head(x)
            flow_map, brightness_map = flow_brightness[:, :2], flow_brightness[:, 2:]
            outs.append(flow_map)
            outs.append(brightness_map)
            return outs
        return x
    ...
    def forward(self, x, angle):
        ...
        flow_brightness = self.flow_brightness_head(x)
        flow_map, brightness_map = flow_brightness[:, :2], flow_brightness[:, 2:]
        return flow_map, brightness_map
